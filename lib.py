...
def dataToPixels(values, vRange, colorGradient, bg=None):
    ...
    __kernel void getImageData(__global uchar *bg, __global float *values, __global uchar *colors, __global uchar *result){
        ...
        // ease out to make low values more visible
        float nnvalue = nvalue - 1.0;
        // nvalue = 1.0 - pow(nnvalue, 4); // quad
        nvalue = 1.0 + pow(nnvalue, 5); // quint
...
def getColorGradient(name="inferno", multiply=255, toInt=True):
    ...
    # ease out the colors to reduce too much black
    dlen = len(data)
    easedIndices = [v for v in range(dlen)]
    for i in range(dlen):
        n = 1.0 * i / (dlen-1)
        # n = pow(n-1, 3) + 1 # cubic
        n = n * (2 - n) # quad
        j = int(round(n * (dlen-1)))
        easedIndices[i] = j
    data = [data[i] for i in easedIndices]
